...
//    public static final Integer MAX_ROWS = 60000;
    ...
    public static void exportEntities(
        ...
        String[] headersChinese1 = { "目标号码", "卡号", "机身码", "姓名", "是否LBS监控(0是不监控，1是监控)", "监控停止时间" };
		String[] headersEnglish1 = { "msisdn", "imsi", "imei", "name", "lbs", "endTime" };
		String[] headersChinese2 = { "区域名称", "来源（1系统内置，2框选，3导入，4录入）" };
		String[] headersEnglish2 = { "areaName", "areaSourceType" };
  ...
  //      insertTitleRow(sheet, headersChinese, cellStyleMap.get("font12Style"));            
        ...
        if (dataList != null && dataList.size() > 0) {
            ...
            Map<String, Method> classMethods1 = getClassMethods(dataList1.get(0));
            Map<String, Method> classMethods2 = getClassMethods(dataList2.get(0));
          ...
          //进行四舍五入取整
            ...
            for(int i=0;i<3;i++){
            	...
            	if(i == 1){
            		nameExcel = "目标号码";
            		headersChinese = headersChinese1;
//            		Map<String, String> map1 = new HashMap<>();
            		headMap.put("sheetTitle", nameExcel);
        			headMap.put("title", nameExcel);
            	}
            	if(i == 2){
            		nameExcel = "目标区域";
            		headersChinese = headersChinese2;
            		headMap.put("sheetTitle", nameExcel);
        			headMap.put("title", nameExcel);
            	}
            ...
            for (Object baseEntity : dataList1) {                  	
           	 HSSFSheet sheet1=list.get(1);//对表格进行初始化
           	 //当第一个表格数据已达上限时，循环遍历出第二个表格，依次循环
           //	 int s=sheet1.getLastRowNum();
//                if((sheet1.getLastRowNum())>=num){
//               	 
...
//                }         		
           	//	System.out.println("excel一行的数据存入");
                   ...
                   HSSFCell cell;
                   HSSFRow newRow = sheet1.createRow(sheet1.getLastRowNum() + 1);
                   for (int i=0;i<headersEnglish1.length;i++) {
                       cell = newRow.createCell(i);
                       try {
                           if (classMethods1.get(headersEnglish1[i].toLowerCase()) != null) {
                               value = classMethods1.get(headersEnglish1[i].toLowerCase()).invoke(baseEntity);
                           }
                           insertCellValue(cell, value, cellStyleMap.get("textStyle"), workbook.getFontAt((short) 0));
                       } catch (SecurityException | IllegalArgumentException | InvocationTargetException | IllegalAccessException e) {
                           e.printStackTrace();
            ...
            for (Object baseEntity : dataList2) {                  	
           	 HSSFSheet sheet1=list.get(2);//对表格进行初始化
           	 //当第一个表格数据已达上限时，循环遍历出第二个表格，依次循环
           //	 int s=sheet1.getLastRowNum();
//                if((sheet1.getLastRowNum())>=num){
//               	 
...
//                }         		
           	//	System.out.println("excel一行的数据存入");
                   ...
                   HSSFCell cell;
                   HSSFRow newRow = sheet1.createRow(sheet1.getLastRowNum() + 1);
                   for (int i=0;i<headersEnglish2.length;i++) {
                       cell = newRow.createCell(i);
                       try {
                           if (classMethods2.get(headersEnglish2[i].toLowerCase()) != null) {
                               value = classMethods2.get(headersEnglish2[i].toLowerCase()).invoke(baseEntity);
                           }
                           insertCellValue(cell, value, cellStyleMap.get("textStyle"), workbook.getFontAt((short) 0));
                       } catch (SecurityException | IllegalArgumentException | InvocationTargetException | IllegalAccessException e) {
                           e.printStackTrace();
